...
def match_template(photos_paths, template_path, method_names):
    ...
    def plot_result():
        ...
        fig.suptitle(main_header, fontsize=20, x=0.2, y=1)
        fig.tight_layout()
...
if __name__ == '__main__':
    ...
    my_template = os.path.join(templates_dir, 'template_8.jpg')
    
    method_names = ['CCOEFF', 'CCOEFF_NORMED', 'CCORR', 'CCORR_NORMED',
                   'SQDIFF', 'SQDIFF_NORMED']
    
    my_photos = os.path.join(orl_dir,'s8', '3.pgm')
    #method_names = 'CCOEFF_NORMED'
    #my_photos = [os.path.join(photos_dir, f'photo_{i+1}.jpg') for i in range(7)]
    #my_photos = [os.path.join(orl_dir, 's11', f'{i+1}.pgm') for i in range(10)]
